...
ccl_af=list(reversed(precio_gal_lista_af))            # lo di vuelta, ya que las fechas venian desde hoy hasta el pasado
ccl_af_1=ccl_af[1:]                                        #lista dos para calcular el aumento con respecto a la semana anterior
ccl_af_aumento=[ccl_af_1[i]/ccl_af[i] for i in range(len(ccl_af_1))]  #aumento con respecto a la semana anterior
...
ccl_mm=list(reversed(precio_gal_lista_mm))            # lo di vuelta, ya que las fechas venian desde hoy hasta el pasado
ccl_mm_1=ccl_mm[1:]                                        #lista dos para calcular el aumento con respecto a la semana anterior
ccl_mm_aumento=[ccl_mm_1[i]/ccl_mm[i] for i in range(len(ccl_mm_1))]  #aumento con respecto a la semana anterior
...
x_ubicacion=porcentuales_mm.index(200.54522782399653)           #numero de semanas de gobierno
y_ubicacion=200.54522782399653
ax.annotate('JULIO-2019', xy=(x_ubicacion,y_ubicacion), xytext=(x_ubicacion-5,y_ubicacion-30),arrowprops=dict(arrowstyle="->"))

x_ubicacion2=porcentuales_mm.index(16.76768621523337)           #numero de semanas de gobierno
y_ubicacion2=16.76768621523337

ax.annotate('DIC-2017', xy=(x_ubicacion2,y_ubicacion2), xytext=(x_ubicacion2-5,y_ubicacion2-40),arrowprops=dict(arrowstyle="->"))

x_ubicacion3=porcentuales_mm.index(352.3990789015343)
y_ubicacion3=352.3990789015343
ax.annotate('SEPT-2017', xy=(x_ubicacion3,y_ubicacion3), xytext=(x_ubicacion3-5,y_ubicacion3-40),arrowprops=dict(arrowstyle="->"))
